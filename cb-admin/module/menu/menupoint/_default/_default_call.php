...
class call extends funct {
    public function init() {
        ...
        if ( $get['funct'] === 'edit' && isset($get['id']) && !empty($get['id']) && is_numeric($get['id']) ) {
            return $this->__default_edit($get['id']);
        } else {
            return $this->__default_new();
        }
    }
    
    private function __default_new() {
        ...
        if ( $this->menupointHtml_saveTest($data) ) {
            ...
            if ( $error ) {
                ...
                $data['catid'] = $catid;
                $data['catlang'] = $catlang;
                $error = $this->menupointHtml_savePost($data,$get['type']);
    ...
    private function __default_edit($id) {
        global $get, $post, $lang;
        ...
        if ( $this->menupointHtml_saveTest($data) ) {
            ...
            if ( $error ) {
                ...
                $data['catid'] = $catid;
                $data['catlang'] = $catlang;
                $error = $this->menupointHtml_updatePost($data,$get['type']);
                if ( $error !== TRUE ) { 
                    $this->menupointHtml_save_error($error);
                } else {
                    $this->menupointHtml_save_success($data);
