...
function ajaxProductImageJS_preview() {
    if ( jQuery('#productimage_file').length === 0 ) { return false; }
    jQuery("#productimage-message-error").hide();
    var prevImg = jQuery('#preview_productimage').attr('src');
    var defFileField = jQuery('#p-productimage_file').html();
    jQuery("#tabs-image").on("change","#productimage_file",function() {
        jQuery("#productimage-message-error").hide();
        var file = this.files[0];
        var imagefile = file.type;
        var match= ["image/jpeg","image/png","image/jpg","image/gif"];
        if(!((imagefile==match[0]) || (imagefile==match[1]) || (imagefile==match[2]) || (imagefile==match[3]))) {
            jQuery('#preview_productimage').attr('src',prevImg);
            jQuery("#p-productimage_file").html(defFileField);
            jQuery("#productimage-message-error").show();
            return false;
        } else {
            jQuery("input[name=productimage_delete]").prop('checked', false);
            var reader = new FileReader();
            reader.onload = imageIsLoaded;
            reader.readAsDataURL(this.files[0]);
        }
    ...
    function imageIsLoaded(e) {
        //jQuery("#file").css("color","green");
        jQuery('#preview_productimage').attr('src', e.target.result);
    };
}
...
function productCreateJS() {
    if ( jQuery('input.productName').length > 0 ) {
        jQuery('input.productName').on("change",function() {
            var lang = jQuery(this).data('lang');
            var name = jQuery(this).val();
            
            if ( jQuery('input.productSEOName.lang_'+lang).val() === '' ) {
        ...
        jQuery('button[name=renewseo]').click(function () {
            var lang = jQuery(this).data('lang');
            var name = jQuery('input.productName.lang_'+lang).val();
...
function seoNameCreate(name,lang,forced) {
    jQuery('input.productSEOName.lang_'+lang).prop('readonly', true);
    var seoname = jQuery('input.productSEOName.lang_'+lang).val();
    ...
    jQuery.getJSON( url, function(e) {
        jQuery('input.productSEOName.lang_'+lang).val(e.seoname);
    }).always(function() {
        jQuery('input.productSEOName.lang_'+lang).prop('readonly', false);
    });
}
